...
3. 9/27 정리: 스터디 내용을 각자 돌아가며 새롭게 알게 된것, 느낀점, 최종적으로 어떤걸 만들고 싶은지 공유
  ...
  b. 각자 03. 정리 폴더에 자신의 이름의 md 파일을 만들어 공유한 내용을 정리하여 기록한다
4. 10/11 실습 관련 논의 : 그룹c, e가 함께 담당하는 프로젝트와 따로 담당하는 프로젝트의 상태관리 개선점에 대해서 논의한다.
   (논검: 냉병기를 통한 대련으로 서로의 무학에 대해 깨달음을 나누는 것)
   논의 후 프로젝트에 적용하여 개선한다. (실습)
```
